...
function sendData (req, res, next) {
  ...
  else if (rep.url == "/usr_update_email") {
    res.send("update email request received");

    var new_email = req.get("new_email");

    return;
  }
  else if (req.url == "/usr_update_password") {
    res.send("update password request received");

    var new_password = res.get("new_password");

    return;
  }
  else if (req.url == "/user_update_all") {
    res.send("update all user information request received");
...
app.post("/usr_update_email", logreq, sendData);
app.post("/usr_update_password", logreq, sendData);
app.post("/user_update_all", logreq, sendData);
